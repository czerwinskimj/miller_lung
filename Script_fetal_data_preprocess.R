...
disMat <- as.matrix(dist(Master@dr$pca@cell.embeddings))
phate.res.k100 <- phate(disMat, knn.dist.method="precomputed", k=100)
#saveRDS(phate.res.k100, file="Res_phate_pseudocell_k100_20PC.rds")
...
#group.res=as.character(as.matrix(info.2[,"Timepoint"]))
group.res=paste0("Cluster", as.character(Master@meta.data$res.0.8))
#group.res=as.vector(as.matrix(pseudo.info["orig.ident",]))
g.cols <- setNames(scales::hue_pal()(length(unique(group.res))), paste0("Cluster",0:max(as.numeric(Master@meta.data$res.0.8))))
#g.cols <- c("#a1d99b","#74c476","#41ab5d","#238b45","#006d2c","#00441b")
#g.cols <- c("#d9d9d9","#bdbdbd","#969696","#737373","#525252","#252525")
#g.cols <- rep("#bdbdbd", 6)
#names(g.cols) <- c("W6", "W12", "W18", "W24", "W30", "W38")
file.format <- "png"
plot.file <- "Plot_cluster_Info_along_PHATE.png"
plot.coor <- phate.res.k100$embedding
#plot.coor <- spring.coor
png(plot.file, height=1000, width=1000)
par(mar=c(6,6,6,6))
plotFeature2(plot.coor, values=group.res, knn.pairs=knn.idx, xlab="", ylab="", main="", cex.lab=1.3, cex.axis=1.3, cex.main=1.5, bty="n", xaxt="n", yaxt="n", gCols=g.cols, cex=1.3)
for(i in unique(group.res)){
	sample.idx <- which(group.res==i)
	g.coor <- as.numeric(apply(plot.coor[sample.idx,], 2, mean))
	text(g.coor[1], g.coor[2], labels=i, cex=1.5)
}
legend("topright", legend=names(g.cols), text.col=g.cols, bty="n")
dev.off()
plot.file="Plot_PHATE_EPCAM_expr.png"
png(plot.file, height=1000, width=1000)
plotFeature2(plot.coor, values=Master@data["EPCAM",], knn.pairs=knn.idx, xlab="", ylab="", main="EPCAM", cex.lab=1.3, cex.axis=1.3, cex.main=1.5, bty="n", xaxt="n", yaxt="n", cex=1.3)
dev.off()
...
#hvg.expr <- as.matrix(dInVivo@data[dInVivo@var.genes,selected.sample])
#write.table(hvg.expr, file="SPRING_vivo_epi_pos/Table_hvg_InVivo_pos_cells_normed_expr.csv", sep=",", col.names=F,quote=F)
#hvg.info <- t(dInVivo@meta.data[selected.sample,])
#write.table(hvg.info, file="SPRING_vivo_epi_pos/Table_hvg_InVivo_pos_info.csv", sep=",", col.names=F,quote=F)

# use cluster markers
epi.vivo.cluster <- cluster.marker[!cluster.marker$cluster%in%c(16,18,19),]
top <- epi.vivo.cluster %>% group_by(cluster) %>% top_n(50, avg_logFC)
selected.markers <- unique(top$gene)
spring.dir <- "SPRING_vivo_epi_cm/"
dir.create(file.path(spring.dir))
hvg.expr <- as.matrix(epi@data[selected.markers,selected.sample])
write.table(hvg.expr, file=paste0(spring.dir, "Table_cm_InVivo_pos_cells_normed_expr.csv"), sep=",", col.names=F,quote=F)
hvg.info <- t(epi@meta.data[selected.sample,])
write.table(hvg.info, file=paste0(spring.dir, "Table_cm_InVivo_pos_info.csv"), sep=",", col.names=F,quote=F)
cor.dis.cm <- 1-cor(hvg.expr)


#spring.coor <- read.csv(paste0(spring.dir,"SPRING_res/coordinates.txt"),head=F,row.names=1)
spring.coor <- read.csv(paste0(spring.dir,"SPRING_k100/coordinates.txt"),head=F,row.names=1)
...
knn.idx <- cbind(as.vector(idx1), rep(seq(ncol(disMat)), each=k))
# define the plotFeature function
plotFeature <- function(coor, values, knn.pairs=NULL, emphasize=NULL, ...){
...
# plot the sample information along kNN network
cluster.res=hvg.info["res.0.8",]
k <- 15
spring.file <- "Plot_filered_InVivo_EpiPos_cluster_Info_along_SPRING_cm-k100-1.png"
png(spring.file, height=1000, width=1000)
par(mar=c(6,6,6,6))
plotFeature(spring.coor, values=cluster.res, knn.pairs=knn.idx, xlab="", ylab="", main="", cex.lab=1.3, cex.axis=1.3, cex.main=1.5, bty="n", xaxt="n", yaxt="n")
...
top1 <- epi.vivo.cluster %>% group_by(cluster) %>% top_n(1, avg_logFC)
g1 <- unique(c(g1, top1$gene))
spring.file <- "Plot_filered_InVivo_EpiPos_cluster_Info_along_SPRING_cm_with_marker_gene.png"
png(spring.file, width=1600, height=5600)
par(mfrow=c(14,4), mar=c(6,6,6,6))
for(x in g1){
	plotFeature(spring.coor, values=epi@data[x,selected.sample], colorScheme="gyr", xlab="SPRING-1", ylab="SPRING-2", main=x, cex.lab=1.5, cex.axis=1.5, cex.main=2)
}
dev.off()


g1 <- c("PDPN", "AGER", "HOPX", "SFTPC", "SOX9", "SFTPB", "CFTR", "SFTPD", "FOXJ1", "SCGB1A1", "SCGB3A1", "SCGB3A2", "TP63", "KRT5", "KRT15", "ASCL1")
spring.file <- "Plot_filered_InVivo_EpiPos_cluster_Info_along_SPRING_cm_with_marker_gene-k100-1.png"

png(spring.file, width=4000, height=4000)
par(mfrow=c(4,4), mar=c(6,6,6,6))
for(x in g1){
	plotFeature(coor=spring.coor, values=epi@data[x,selected.sample], knn.pairs=knn.idx, xlab="", ylab="", main=x, cex.lab=1.3, cex.axis=1.3, cex.main=5, bty="n", xaxt="n", yaxt="n", cex=1.2)
}
dev.off()

spring.file <- "Plot_filered_InVivo_EpiPos_age_and_location_Info_along_SPRING_cm-k100-1.png"
...
# check the PCA 
spring.file <- "Plot_filered_InVivo_EpiPos_PC_Info_along_SPRING_cm-k100-1.png"
png(spring.file, width=4000, height=5000)
par(mfrow=c(5,4), mar=c(6,6,6,6))
for(i in seq(20)){
	plotFeature(coor=spring.coor, values=epi@dr$pca@cell.embeddings[,i], xlab="", ylab="", main=paste0("PC",i), cex.lab=1.3, cex.axis=1.3, cex.main=5, bty="n", xaxt="n", yaxt="n", cex=1.2)
}
dev.off()

# prepare input for scanorama
count.dir <- "epi_filtered_raw_count/"
dir.create(file.path(count.dir))

for(x in samples){
	count <- as.matrix(epi@raw.data[,grep(x, selected.sample, value=T)])
	output <- paste0(count.dir, "Table_epi_filtered_",x,"_count.txt")
	write.table(count, file=output, quote=FALSE, sep="\t")
}

# read scanorama-corrected values
sca.res <- lapply(samples, function(x){
	cat(paste(x, "start\n"))
	file <- paste0(count.dir, "Table_epi_filtered_",x,"_count.scanorama_corrected.txt")
	dat <- read.table(file, sep="\t", stringsAsFactors=F, row.names=1, head=T)
	colnames(dat) <- grep(x, selected.sample, value=T)
	return(dat)
})
sca.corrected <- do.call("cbind", sca.res)
raw.count <- epi@raw.data[rownames(sca.corrected), selected.sample]
scale.data <- t(scale(t(sca.corrected[,selected.sample])))
sca.corrected <- CreateSeuratObject(raw.data=raw.count)
sca.corrected <- NormalizeData(object=sca.corrected)
sca.corrected@scale.data <- scale.data
sca.corrected <- FindVariableGenes(object=sca.corrected, do.plot=F)
sca.corrected <- RunPCA(object=sca.corrected, pc.genes=sca.corrected@var.genes, do.print=FALSE)
sca.corrected <- FindClusters(object=sca.corrected, reduction.type="pca", dims.use=1:20, resolution=0.8, print.output=0, save.SNN=TRUE, force.recalc=TRUE)
sca.corrected <- RunTSNE(object=sca.corrected, dims.use=1:20, do.fast=TRUE, check_duplicates=F)


p2 <- TSNEPlot(object=sca.corrected, pt.size=1, do.return=T, do.label=T,label.size=5)+theme(axis.text=element_text(size=15), axis.ticks.length=unit(.25, "cm"), axis.title=element_text(size=18),legend.text=element_text(size=15))
p3 <- TSNEPlot(object=sca.corrected, pt.size=1, group.by="orig.ident", do.return=T, label.size=5)+theme(axis.text=element_text(size=15), axis.ticks.length=unit(.25, "cm"), axis.title=element_text(size=18),legend.text=element_text(size=15))
png("Plot_tSNE_res0.8_afterSca_scaled.png",width=1000, height=500)
plot_grid(p2, p3, ncol=2, nrow=1)
dev.off()
saveRDS(sca.corrected, file="Dat_scaled_scaCorrected_epi_filtered.rds")

cluster.marker <- FindAllMarkers(object=sca.corrected, only.pos=T, min.pct=0.25, thresh.use=0.25)
saveRDS(cluster.marker, file="Res_scaled_scaCorrected_Epi_cluster_marker.rds")

library(dplyr)
top <- cluster.marker %>% group_by(cluster) %>% top_n(5, avg_logFC)
png("Plot_scaCorrected_EpCAM_positive_subclustering_top5_cluster_marker.png",height=1000,width=2000)
DoHeatmap(object = sca.corrected, genes.use = top$gene, slim.col.label = TRUE, remove.key = TRUE)
dev.off()

g1 <- intersect(markers[,2], rownames(sca.corrected@data))
png("Plot_scaled_scaCorrected_marker_expression_on_epi_tSNE.png", width=1600, height=4400)
FeaturePlot(object=sca.corrected, features.plot=g1, cols.use=c("gray", "blue"), no.legend=F)
dev.off()

vivo.cl.ave.expr <- foreach(cl.idx=unique(cluster.marker$cluster), .combine='cbind', .multicombine=T)%dopar%{
	cat(paste("Cluster", cl.idx, "start\n"))
	cl.sample.idx <- which(sca.corrected@meta.data$res.0.8==cl.idx)
	e <- apply(sca.corrected@data[,cl.sample.idx],1,mean)
	return(e)
}
colnames(vivo.cl.ave.expr) <- paste0("Cluster", unique(cluster.marker$cluster))
saveRDS(vivo.cl.ave.expr, file="Dat_scaCorrected_InVivo_cluster_ave_expr.rds")


# prepare matrix for SPRING
# use cluster markers
top <- cluster.marker %>% group_by(cluster) %>% top_n(50, avg_logFC)
selected.markers <- unique(top$gene)
spring.dir <- "SPRING_scaled_scaCorrected_epi_cm/"
dir.create(file.path(spring.dir))
hvg.expr <- as.matrix(sca.corrected@scale.data[selected.markers,])
write.table(hvg.expr, file=paste0(spring.dir, "Table_data.csv"), sep=",", col.names=F,quote=F)
hvg.info <- t(sca.corrected@meta.data)
write.table(hvg.info, file=paste0(spring.dir, "Table_meta_data.csv"), sep=",", col.names=F,quote=F)
disMat <- 1-cor(hvg.expr)

# use PCs
spring.dir <- "SPRING_scaled_scaCorrected_epi_pc/"
dir.create(file.path(spring.dir))
hvg.expr <- as.matrix(t(sca.corrected@dr$pca@cell.embeddings))
write.table(hvg.expr, file=paste0(spring.dir, "Table_data.csv"), sep=",", col.names=F,quote=F)
hvg.info <- t(sca.corrected@meta.data)
write.table(hvg.info, file=paste0(spring.dir, "Table_meta_data.csv"), sep=",", col.names=F,quote=F)
...
spring.coor <- read.csv(paste0(spring.dir,"SPRING_sca_pc/coordinates.txt"),head=F,row.names=1)
rownames(spring.coor) <- colnames(hvg.info)
# get the kNN network
k <- 15
idx1 <- apply(disMat, 2, function(vec){
...
knn.idx <- cbind(as.vector(idx1), rep(seq(ncol(disMat)), each=k))
# define the plotFeature function
plotFeature <- function(coor, values, knn.pairs=NULL, emphasize=NULL, nCols=NULL, gCols=NULL, ...){
	if(is.numeric(values)){
		if(is.null(nCols)){
			colorPal <- grDevices::colorRampPalette(c("darkgreen", "yellow","red"))
		}else{
...
g1 <- c("PDPN", "AGER", "HOPX", "SFTPC", "SOX9", "SFTPB", "CFTR", "SFTPD", "FOXJ1", "SCGB1A1", "SCGB3A1", "SCGB3A2", "TP63", "KRT5", "KRT15", "ASCL1")
g2 <- intersect(g1, rownames(sca.corrected@data))
spring.file <- "Plot_SPRING_scaled_sca_pc_marker_gene.png"

png(spring.file, width=4000, height=4000)
par(mfrow=c(4,4), mar=c(6,6,6,6))
for(x in g2){
	plotFeature(coor=spring.coor, values=sca.corrected@data[x,], knn.pairs=knn.idx, xlab="", ylab="", main=x, cex.lab=1.3, cex.axis=1.3, cex.main=5, bty="n", xaxt="n", yaxt="n", cex=1.2)
}
dev.off()

spring.file <- "Plot_filered_InVivo_EpiPos_age_and_location_Info_along_SPRING_pc_scaled_sca.png"
age.idx <- rep(NA, ncol(hvg.info))
age.idx[grep("D145", hvg.info["orig.ident",])] <- "Day145"
age.idx[grep("D103", hvg.info["orig.ident",])] <- "Day103"
age.idx[grep("D125", hvg.info["orig.ident",])] <- "Day125"
...
hvg.gene.idx <- matrix(F, nrow=nrow(epi@data), ncol=length(samples))
rownames(hvg.gene.idx) <- rownames(epi@data)
colnames(hvg.gene.idx) <- samples
for(j in seq(length(samples))){
	hvg.gene.idx[which(rownames(hvg.gene.idx)%in%epi.by.sample[[j]]@var.genes),j] <- T
}
selected.hvg <- names(which(apply(hvg.gene.idx, 1, sum)>1))
save(selected.hvg, hvg.gene.idx, file="epi_by_sample/Res_hvg_by_sample.rdata")
#selected.hvg <- names(which(apply(hvg.gene.idx, 1, sum)==6))
#hvg.dis <- names(which(apply(hvg.gene.idx[,c("D125dis", "D103dis")], 1, sum)>1))
#hvg.mid <- names(which(apply(hvg.gene.idx[,c("D125prox", "D103air")], 1, sum)>1))
#hvg.prox <- names(which(apply(hvg.gene.idx[,c("D103tra", "D145tra")], 1, sum)>1))
#selected.hvg <- intersect(union(hvg.dis, hvg.mid), hvg.prox)

epi <- RunPCA(object=epi, pc.genes=selected.hvg, do.print=FALSE)
epi <- FindClusters(object=epi, reduction.type="pca", dims.use=1:20, resolution=2, print.output=0, save.SNN=TRUE, force.recalc=TRUE)
epi <- RunTSNE(object=epi, dims.use=1:20, do.fast=TRUE)

p2 <- TSNEPlot(object=epi, pt.size=1, do.return=T, do.label=T,label.size=5)+theme(axis.text=element_text(size=15), axis.ticks.length=unit(.25, "cm"), axis.title=element_text(size=18),legend.text=element_text(size=15))
p3 <- TSNEPlot(object=epi, pt.size=1, group.by="orig.ident", do.return=T, label.size=5)+theme(axis.text=element_text(size=15), axis.ticks.length=unit(.25, "cm"), axis.title=element_text(size=18),legend.text=element_text(size=15))

merged.cl <- rep(0, nrow(epi@meta.data))
names(merged.cl) <- rownames(epi@meta.data)
merged.cl[colnames(hvg.info)] <- as.numeric(as.matrix(hvg.info["merged.cl",]))
epi@meta.data <- cbind(epi@meta.data, merged.cl)
p4 <- TSNEPlot(object=epi, pt.size=1, group.by="merged.cl", do.return=T, do.label=T, label.size=5)+theme(axis.text=element_text(size=15), axis.ticks.length=unit(.25, "cm"), axis.title=element_text(size=18),legend.text=element_text(size=15))
png("epi_by_sample/Plot_tSNE_epi_merged_clusters_hvg.png",width=1500, height=800)
...
vivo.cl.ave.expr.2 <- foreach(cl.idx=0:max(as.numeric(epi@meta.data$res.2)), .combine='cbind', .multicombine=T)%dopar%{
	cat(paste("Cluster", cl.idx, "start\n"))
	cl.sample.idx <- which(epi@meta.data$res.2==cl.idx)
	e <- apply(epi@data[c("EPCAM","EPCAM"),cl.sample.idx],1,mean)
	return(e)
}
stopImplicitCluster()
colnames(vivo.cl.ave.expr) <- paste0("Cluster", 0:max(as.numeric(epi@meta.data$res.2)))
hc <- hclust(as.dist(1-cor(vivo.cl.ave.expr[selected.hvg,])), method="ward.D2")
pdf("epi_by_sample/Plot_hc_ward2.pdf")
plot(hc)
dev.off()
...
png("epi_by_sample/Plot_PCA_coordinates_on_tSNE.png", width=4000, height=5000)
par(mfrow=c(5,4), mar=c(6,6,6,6))
for(i in seq(20)){
	plotFeature(coor=tsne.coor, values=epi@dr$pca@cell.embeddings[,i], xlab="", ylab="", main=paste0("PC",i), cex.lab=1.3, cex.axis=1.3, cex.main=5, bty="n", xaxt="n", yaxt="n", cex=1.2)
}
dev.off()
	...
	g1 <- markers[,2]
	png("epi_by_sample/Plot_marker_expression_on_tSNE_hvg.png", width=1600, height=4800)
	FeaturePlot(object=epi, features.plot=g1, cols.use=c("gray", "blue"), no.legend=F)
	dev.off()
	cluster.marker <- FindAllMarkers(object=epi, only.pos=T, min.pct=0.25, thresh.use=0.25)
	saveRDS(cluster.marker, file="epi_by_sample/Res_Epi_cluster_marker.rds")
top <- cluster.marker %>% group_by(cluster) %>% top_n(5, avg_logFC)
png("epi_by_sample/Plot_EpCAM_positive_subclustering_top5_cluster_marker.png",height=1000,width=2000)
DoHeatmap(object = epi, genes.use = top$gene, slim.col.label = TRUE, remove.key = TRUE)
dev.off()

saveRDS(epi, file="epi_by_sample/Res_epi.rds")

# merge clusters
cl.res <- epi@meta.data$res.2
merged.cl <- rep(0, length(cl.res))
merged.cl[which(cl.res%in%c(4,9,8,15))] <- 1
merged.cl[which(cl.res==11)] <- 2
merged.cl[which(cl.res==24)] <- 3
merged.cl[which(cl.res%in%c(26,2,14))] <- 4
merged.cl[which(cl.res==20)] <- 5
merged.cl[which(cl.res%in%c(1,7))] <- 6
merged.cl[which(cl.res%in%c(23,25,27))] <- 0
merged.cl[which(cl.res==22)] <- 7
merged.cl[which(cl.res%in%c(3,5,18))] <- 8
merged.cl[which(cl.res%in%c(10,12,13))] <- 9
merged.cl[which(cl.res%in%c(6,17,19))] <- 10
merged.cl[which(cl.res==21)] <- 11
merged.cl[which(cl.res==0)] <- 12
merged.cl[which(cl.res==16)] <- 13
epi@meta.data <- cbind(epi@meta.data, merged.cl)
...
# use PCs
spring.dir <- "SPRING_merged_pc_scaled_regressed/"
dir.create(file.path(spring.dir))
hvg.expr <- as.matrix(t(epi@dr$pca@cell.embeddings[selected.samples, ]))
write.table(hvg.expr, file=paste0(spring.dir, "Table_data.csv"), sep=",", col.names=F,quote=F)
hvg.info <- t(epi@meta.data[selected.samples, ])
write.table(hvg.info, file=paste0(spring.dir, "Table_meta_data.csv"), sep=",", col.names=F,quote=F)
disMat <- as.matrix(dist(t(hvg.expr)))
...
# check the marker genes separating different ciliated cells
